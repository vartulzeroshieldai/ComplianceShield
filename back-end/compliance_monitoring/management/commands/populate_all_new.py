...
class Command(BaseCommand):
    help = 'Populates the database with all new compliance frameworks from CSV files.'

    def add_arguments(self, parser):
        parser.add_argument('--framework', type=str, help='Specific framework to populate.')
    ...
    def handle(self, *args, **options):
        self.stdout.write("🚀 Starting population of all new frameworks...")
        
        # Show current state
        self.show_current_frameworks()
        
        # Process frameworks based on options
        if options['framework']:
            # Find the framework by name
            target_config = None
            target_file = None
            for csv_file, config in self.framework_configs.items():
                if options['framework'].lower() in config['name'].lower():
                    target_config = config
                    target_file = csv_file
                    break
            
            if target_config:
                self.populate_framework(target_file, target_config, options['clear_existing'])
            else:
                self.stdout.write(self.style.ERROR(f"Framework not found: {options['framework']}"))
        else:
            ...
            for csv_file, config in self.framework_configs.items():
                if self.populate_framework(csv_file, config, options['clear_existing']):
                    success_count += 1
            
            self.stdout.write(f"\n=== Final Results ===")
            self.stdout.write(f"✅ Successfully processed: {success_count}/{total_count} frameworks")
    ...
    def populate_framework(self, csv_file, config, clear_existing=False):
        ...
        framework_exists = Framework.objects.filter(name=framework_name).exists()
        ...
        if framework_exists:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Creating new framework: {framework_name}"))
        ...
        # Read CSV file
        csv_path = os.path.join(settings.BASE_DIR, 'compliance_monitoring', csv_file)
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Error processing {csv_file}: {str(e)}"))
            return False
        
        self.stdout.write(f"   📊 Results: {clauses_created} clauses, {subclauses_created} sub-clauses, {skipped} skipped")
        return True
